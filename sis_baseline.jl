...
function next_event_time(residence_time::Real, target_state::State, neighbor_restimes::NeighborhoodTimes, neighbor_states::NeighborhoodStates, horizon::Real)::Tuple{Real,Real}
    ...
    if target_state == Infected()
        t_e = 0.0
        mu::Real = 0.4
        number_inf_neighbors = length([s for (n,s) in neighbor_states if s==Infected()])
        if number_inf_neighbors == 0
            return 10000.0+rand(), 100000.0 #Dummy
        end
        over_approx_rate::Real = mu*number_inf_neighbors
        while true
            if t_e  > horizon
            ...
            t_e += t_e_shift
            actual_rate = get_actual_inf_rate(t_e, neighbor_restimes,neighbor_states)
            #println(actual_rate," vs ",over_approx_rate)
